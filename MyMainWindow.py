...
# import threading
import time
from PyQt5.QtGui import QPainter, QPen, QPainterPath, QBrush, QFontMetrics  # , QPalette, QColor
import math
...
class MyMainWindow(QtWidgets.QMainWindow):
    def __init__(self, workerThread, mouseClickCallable=None, scrollWheelCallable=None, sides=(106, 20), *args, **kwargs):
        ...
        up_threshold = kwargs.pop('up_threshold', 0)
        down_threshold = kwargs.pop('down_threshold', 0)
        # Set the opacity of the Window. The cursor movement over the Window will again make it opaque
        opacity = kwargs.pop('opacity', 0.7)
        pos = kwargs.pop("pos", None)
        font = kwargs.pop('font', {"family": "Lexend", "pointSize": 6, "weight": 35})
        # Set window always on top, with frameless window for better view and start as tool to avoid Taskbar Icon
        ...
        # Add the actions for mouse Right Click
        self.context_menu.addAction(self.set_on_top_action)
        self.context_menu.addAction(self.refresh_action)
        self.context_menu.addAction(self.close_window)
        # Mouse enter Event
        self.central_widget = QtWidgets.QWidget(self)
        self.setCentralWidget(self.central_widget)
        self.ui = Ui_Dialog(self, workerThread, mouseClickCallable, scrollWheelCallable, *args, **kwargs)
...
class Ui_Dialog():
    def __init__(self, MainWindow, workerThread, mouseClickCallable, scrollWheelCallable, font={}, *args, **kwargs):
        ...
        self.setupUi(MainWindow, *args, **kwargs)
        MainWindow.refresh_action.triggered.connect(self.refresh_action)
        MainWindow.close_window.triggered.connect(self.workerThread.stop)
    ...
    def setupUi(self, MainWindow, *args, **kwargs):
        ...
        font = kwargs.get("font", {"family": "Sofia Pro", "pointSize": 7, "weight": 75})
        family = font.pop("family", "Sofia Pro")
        # self.label.setStyleSheet("color: white; border: 2px solid black;")
        # font = QtGui.QFont("ia-writer-duospace", 7)
        font = QtGui.QFont(family, **font)
    ...
    def refresh_action(self):
        if hasattr(self.workerThread, 'refresh'):
    ...
    def mouseWheelEvent(self, event, source_obj):
        ...
        # Performs a callable for up even, if the callable exists for up and down events. The callable can be defined by the app using this Framework
...
class CustomQLabel(QtWidgets.QLabel):
    def __init__(self, font, *args, **kwargs):
        ...
        self.setTextFormat(QtCore.Qt.TextFormat.AutoText)
    ...
    def get_link(self, line: int, delim='\n'):
        ...
        NOTE: Currently, due to issues, the link part should be verified on implementation. Mostly, it will work,
        ....: but if the change of window text and click coincides, the result may not come as expected
...
class BaseWorkerThread(QThread, QObject):
    """
    Use this to create and extend your application for running as a thread on the current Framework
    """
